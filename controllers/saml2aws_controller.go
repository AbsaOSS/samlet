...
func (r *Saml2AwsReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	saml.Status.ExpirationTime = metav1.Time{Time: time.Now()}
	if needsUpdate(saml) {
		creds, profile, err := r.createAWSCreds(saml)
		if err != nil {

			return ctrl.Result{}, r.updateStatus(ctx, saml, err)
		}
		...
		case "credentialsFile":
			...
			if err != nil {
				err = fmt.Errorf("failed to generate ini file")
				return ctrl.Result{}, r.updateStatus(ctx, saml, err)
			}
		case "envVariables":
...
func (r *Saml2AwsReconciler) updateStatus(ctx context.Context, saml *samletv1.Saml2Aws, err error) error {
	if err != nil {
		saml.Status.State = err.Error()
	}
	return r.Status().Update(ctx, saml)
}
